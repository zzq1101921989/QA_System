  ...
  return (
    <div className="p-4 md:p-8 pt-0 flex-shrink-0">
      <div className="max-w-4xl mx-auto relative group">
        <div className="absolute -inset-0.5 bg-gradient-to-r from-lab-accent/20 to-blue-500/20 rounded-2xl blur opacity-30 group-focus-within:opacity-100 transition-opacity" />
        <div className="relative flex items-center gap-2 p-2 bg-lab-panel border border-lab-border rounded-2xl focus-within:border-lab-accent/50 transition-all shadow-2xl">
          <input 
            ...
            className="flex-1 bg-transparent border-none outline-none px-2 md:px-4 text-xs md:text-sm text-white placeholder:text-zinc-600"
          />
          <button 
            onClick={onSend}
            disabled={!input.trim()}
        ...
        <div className="mt-2 md:mt-3 flex justify-between px-2">
          <div className="flex gap-2 md:gap-4">
            <span className="text-[8px] md:text-[9px] font-mono text-zinc-600 uppercase tracking-tighter flex items-center gap-1">
          ...
          {!isMobile && <span className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">按下 Enter 发送请求</span>}
        </div>
      </div>
    </div>
  );
